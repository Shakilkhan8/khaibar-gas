...
class ShIrAttachment(models.Model):
    ...
    @api.model
    def notify_account_document_expiry(self):

        template = self.env.ref(
            'sh_account_document.sh_account_document_expiry_notify_email')
        notify_create_user_template = self.env.ref(
            'sh_account_document.sh_account_document_expiry_notify_email_to_user')
        ...
        if template and company_object and company_object.sh_expiry_notification:
            ...
            if document_obj:
                for record in document_obj:
                    ...
                    invoice = self.env['account.move'].browse(record.res_id)
                    record.write({
