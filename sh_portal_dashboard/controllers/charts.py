...
class Charts(http.Controller):
    ...
    def get_sale_chart(self, sh_filter_chart_type=False, **kw):
        ...
        website = self.default_company()
        is_installed_sale = request.env['ir.module.module'].sudo().search(
            [('name', '=', 'sale'), ('state', '=', 'installed')]).id
        if is_installed_sale and website.sh_portal_dashboard_is_show_sale_chart and request.env.user.partner_id.customer_rank > 0:
    ...
    def get_invoice_chart(self, sh_filter_chart_type=False, **kw):
        ...
        if is_installed_account and website.sh_portal_dashboard_is_show_invoice_chart and request.env.user.partner_id.customer_rank > 0:
            values.update({
    ...
    def get_purchase_chart(self, sh_filter_chart_type=False, **kw):
        ...
        if is_installed_purchase and website.sh_portal_dashboard_is_show_purchase_chart and request.env.user.partner_id.supplier_rank > 0:
            values.update({
    ...
    def set_bill_charts(self, sh_filter_chart_type=False, **kw):
        ...
        is_installed_account = request.env['ir.module.module'].sudo().search(
            [('name', '=', 'account'), ('state', '=', 'installed')]).id
        if is_installed_account and website.sh_portal_dashboard_is_show_bill_chart and request.env.user.partner_id.supplier_rank > 0:
            values.update({
    ...
    def get_product_wise_sale_data(self, sh_filter_chart_type=False, **kw):
        ...
        if is_installed_sale and website.sh_portal_dashboard_is_show_sale_chart and request.env.user.partner_id.customer_rank > 0:
            ...
            domain = [
                ('order_id.message_partner_ids', 'child_of',
                 [partner.commercial_partner_id.id]),
                    ...
                    if line.product_id.sudo():
                        if products_dic.get(line.product_id.sudo().id, False):
                            ...
                            products_dic.update({
                                ...
                                ).name, 'qty': qty, 'amount': amount, 'img': line.product_id.image_128}
                            })
                        else:
                            products_dic.update({
                                line.product_id.sudo().id: {'product': line.product_id.sudo(
                                ).name, 'qty': line.product_uom_qty, 'amount': line.price_subtotal, 'img': line.product_id.image_128}
                            })
                ...
                for line in order_lines:
                    if line.product_id.sudo():
                        if products_dic.get(line.product_id.sudo().id, False):
                            ...
                            products_dic.update({
                                line.product_id.sudo().id: {'product': line.product_id.sudo(
                                ).name, 'qty': line.qty_received, 'amount': line.price_subtotal, 'img': line.product_id.image_128}
                            })
    ...
    def get_project_task_data(self, sh_filter_chart_type=False, **kw):
        ...
        if is_installed_project and website.sh_portal_dashboard_is_show_project_task_chart:
            ...
            tasks = project_task_obj.sudo().search(domain)
            if tasks:
                for task in tasks:
                    task_url = """
    ...
    def get_last_quote(self, sh_filter_chart_type=False, **kw):
        ...
        is_installed_sale = request.env['ir.module.module'].sudo().search(
            [('name', '=', 'sale'), ('state', '=', 'installed')]).id
        if is_installed_sale and website.sh_portal_dashboard_is_show_last_quote_table and request.env.user.partner_id.customer_rank > 0:
            ...
            sale_orders = sale_order_obj.sudo().search(domain, order="date_order desc",
                                                       limit=website.sh_portal_dashboard_is_show_last_record_limit)
    ...
    def get_last_sale_order(self, sh_filter_chart_type=False, **kw):
        ...
        website = self.default_company()
#         website = self._default_website()
        order_list = []

        is_installed_sale = request.env['ir.module.module'].sudo().search(
            [('name', '=', 'sale'), ('state', '=', 'installed')]).id
        if is_installed_sale and website.sh_portal_dashboard_is_show_last_sale_order_table and request.env.user.partner_id.customer_rank > 0:
    ...
    def get_last_rfq(self, sh_filter_chart_type=False, **kw):
        ...
        is_installed_purchase = request.env['ir.module.module'].sudo().search(
            [('name', '=', 'purchase'), ('state', '=', 'installed')]).id
        if is_installed_purchase and website.sh_portal_dashboard_is_show_last_rfq_table and request.env.user.partner_id.supplier_rank > 0:
    ...
    def get_last_purchase_order(self, sh_filter_chart_type=False, **kw):
        ...
        is_installed_purchase = request.env['ir.module.module'].sudo().search(
            [('name', '=', 'purchase'), ('state', '=', 'installed')]).id
        if is_installed_purchase and website.sh_portal_dashboard_is_show_last_purchase_order_table and request.env.user.partner_id.supplier_rank > 0:
    ...
    def get_last_invoice(self, sh_filter_chart_type=False, **kw):
        ...
        is_installed_account = request.env['ir.module.module'].sudo().search(
            [('name', '=', 'account'), ('state', '=', 'installed')]).id
        if is_installed_account and website.sh_portal_dashboard_is_show_last_invoice_table and request.env.user.partner_id.customer_rank > 0:
    ...
    def get_last_bill(self, sh_filter_chart_type=False, **kw):
        ...
        is_installed_account = request.env['ir.module.module'].sudo().search(
            [('name', '=', 'account'), ('state', '=', 'installed')]).id
        if is_installed_account and website.sh_portal_dashboard_is_show_last_bill_table and request.env.user.partner_id.supplier_rank > 0:
